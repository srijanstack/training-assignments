function Todo({ doingTask, setItem, item }) {
  return (
    <>
      <div className="min-w-[80%] gap-4 h-auto flex flex-col grow items-start justify-around">
        ...
        <Todolist doingTask={doingTask} setItem={setItem} item={item} />
      </div>
    </>
  );
}

function Todolist({ doingTask, setItem, item }) {
  function handledChange(i) {
    const updatedTask = item.map((task) =>
      task.id === i.id ? { ...task, completed: true } : task
    );
    setItem(updatedTask);
  }
