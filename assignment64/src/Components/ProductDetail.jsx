...
function Details({ setCart, cart }) {
  ...
  useEffect(() => {
    ...
    p.then((res) =>{setProduct(res);setLoading(false)}).catch(()=>setLoading(true));
    setQuant(1);
    setAdded(false);
        ...
        <div className="h-[650px] w-[400px] sm:w-[450px] md:w-[600px] md:h-[650px] lg:h-[450px] lg:w-[1000px] border-2 border-black rounded-xl p-5 flex justify-around bg-white flex-col lg:flex-row gap-1">
          ...
          <div className="lg:w-[50%]  w-full lg:py-2 p-0">
            ...
            <div className="flex justify-around h-[45px] w-[180px]  items-center mt-4 ">
              {product.id != 1 && <Link to={`/products/${product.id - 1}`} className="bg-[#676767] h-[80%] w-[45%] rounded-3xl flex items-center justify-center"><AiOutlineArrowLeft className="text-white text-2xl" /></Link>}
              <Link to={`/products/${product.id + 1}`} className="bg-[#676767] h-[80%] w-[45%] rounded-3xl flex items-center justify-center"><AiOutlineArrowRight className="text-white text-2xl"  /></Link>
            </div>
          </div>
        </div>
        :
