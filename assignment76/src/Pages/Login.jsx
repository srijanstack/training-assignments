...
import Input from "../Components/NormalInput";
import axios from "axios";
import { MdAlternateEmail } from "react-icons/md";
import Alert from "../Components/Alert";
import {withAlert, withUser} from "../Components/HOC/WithPorvider";
...
function handleLogin(values, bag) {
  axios
    ...
    .then((res) => {
      ...
      bag.props.setAlert({ message: " User logged in", type: "Success!" });
    })
    .catch(() =>
...
export function Loginform({
  ...
  return (
    <>
      <div className="h-auto  w-full bg-backgrey flex items-center justify-center py-10 flex-grow flex-col gap-2">
                    <Alert></Alert>
        {user ? (
...
function UserLoggedIn({ user, handleLogOut }) {
  return (
    <>
      <div className="min-h-[80vh] max-h-[80vh]  min-w-[80vw] bg-[blue] bg-cover flex flex-col items-center py-4 gap-10 rounded-3xl justify-center">
        <div className="flex gap-2 flex-col items-center justify-center">
          <h1 className="text-white text-md">User Logged In:</h1>
          <h1 className="text-white text-4xl">{user.firstName}</h1>
        </div>
